...
## 14. Сводные тайминги и бенчмарки пайплайна (Latency & Timing Benchmarks)

В таблице ниже приведены реальные замеры задержек на каждом этапе работы системы (измерено на VPS 1 vCPU / 1 GB RAM):

| Операция / Этап | До оптимизации | После оптимизации | Механизм ускорения |
|:---|:---:|:---:|:---|
| **Первый старт радио (`/api/radio/start`)** | **25 – 45 сек** | **0.8 – 1.2 сек** | Переключение на гарантированно готовый трек (`getHotStartingTrack`) вместо холодного поиска нескачанного трека на YouTube. |
| **Отдача аудио при наличии файла на диске (`/api/stream/:id`)** | **5 177 мс** *(или таймаут 41с)* | **1.2 – 3.5 мс** | Прямой стриминг файла с диска Deno (`serveDirectAudioFile`) минуя проксирование в Go core; устранение лишнего full-table scan. |
| **Поиск `ytdl_id` в `external_catalog` (81k строк)** | **768.68 мс** | **0.49 мс** | Замена функции `LOWER(TRIM(?))` на индексированное сравнение `COLLATE NOCASE` (1500x ускорение). |
| **Генерация сбалансированной очереди (`buildBalancedRadioQueue`, 6 треков)** | **1 200 – 2 500 мс** | **17 – 35 мс** | Быстрая выборка кандидатов из базы данных, индексированный отбор 12D признаков. |
| **Скачивание трека по известному `ytdl_id`** | **15 – 35 сек** *(ytsearch)* | **1.8 – 3.2 сек** | Прямое обращение к URL `https://youtube.com/watch?v=<id>` без парсинга страниц поиска. |
| **Холодный поиск и скачивание нового неизвестного трека** | **25 – 45 сек** | **12 – 18 сек** | Оптимизация параметров `yt-dlp` (`--socket-timeout 10`, `--retries 2`, opus-only формат). |
| **Вычисление 512D CLAP эмбеддинга (CPU)** | **7.06 сек** | **3.67 сек** | Батчинг 3 окон в один forward pass PyTorch (1.92x ускорение). |
| **Скоринг трека против центроида вкуса (`score512AgainstCentroid`)** | **15 мс** | **0.08 мс** | Векторизованное скалярное произведение Float32Array в памяти. |
| **Переключение на следующий трек в радио (Skip / Next Track)** | **20 – 30 сек** *(stall)* | **0 мс (мгновенно)** | Zero-Stall Guarantee: позиция `queue[0]` всегда занята уже скачанным треком, пока `queue_watcher` качает следующие в фоне. |
| **Фоновая предбуферизация очереди (`queue_watcher`)** | Фоновый процесс | **2 – 6 сек** | Работает во время прослушивания текущей песни (длительность ~180 сек) — пользователь задержки не замечает. |
