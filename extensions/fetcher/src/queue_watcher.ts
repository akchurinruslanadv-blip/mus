...
export async function handleQueueChange(): Promise<void> {
  ...
  // Pre-buffer only the top 1 upcoming track in queue
  ...
  for (const item of upcoming) {
    ...
    if (!exists) {
      ...
      console.log(`[queue-watcher] Pre-buffering upcoming track: "${query}" (ID: ${item.track_id})...`);

      const result = await fetchAudioStream(query, { mode: "cache" });
